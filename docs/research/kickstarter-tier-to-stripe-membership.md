...
**No Stripe-side redemption mechanism is needed for the straggler tail. A manual Founder stamp in Supabase is enough, and is strictly less work than any Stripe path.**

The reasoning is structural, not preferential. A Stripe promotion code is redeemed by a **Stripe Customer inside a Checkout Session**, and `stripe-checkout-edge.ts` only mints a Stripe customer for a caller who already holds a verified Supabase session. So a code cannot be redeemed by anyone who lacks a PRISM account — the exact population it was supposed to serve. Both the code path and the allowlist path require the backer to make an account first; past that point the code buys nothing the allowlist does not already give, and costs a coupon, a promotion code per backer, a new `discounts[0][promotion_code]` parameter in the checkout edge function, a redemption UI, and a permanent second entitlement source that PRISM must keep reconciled with Stripe. The Founder stamp costs one row.

The rest follows from that:

- **PRISM's entitlement gate is already PRISM's own.** `isPaymentEnforced()` reads `app_config`; `hasActiveSubscription()` reads the local `subscriptions` row. A Founder flag is read by the same code with no Stripe object behind it. Sending a backer who is already a Founder through Checkout creates a live Stripe subscription that PRISM then owes maintenance to forever — trial-end handling, pause handling, dunning — in exchange for an entitlement it could have granted by writing `true` into a column.
...
Cost: "There is no up front cost to using this tool. Kickstarter will deduct our usual fees from any payments made within the pledge manager" ([How do I use Kickstarter's Pledge Manager?](https://help.kickstarter.com/hc/en-us/articles/30353536404891-How-do-I-use-Kickstarter-s-Pledge-Manager)). Delivering a per-backer code is therefore free and needs no third party. Eligibility was gated when this note was written and is not any longer. **Verified 2026-09-04:** Kickstarter opened the tool to everyone on 2025-05-08 — *“Starting today, the Kickstarter Pledge Manager is available to all creators”* ([Bringing the Kickstarter Pledge Manager to all creators](https://updates.kickstarter.com/bringing-the-kickstarter-pledge-manager-to-all-creators/)) — and the eligibility article now opens *“All projects are eligible to use the Kickstarter Pledge Manager tool, provided your project’s rewards have all been properly itemized”* ([How do I know if I’m eligible to use Kickstarter’s Pledge Manager?](https://help.kickstarter.com/hc/en-us/articles/36724482042139-How-do-I-know-if-I-m-eligible-to-use-Kickstarter-s-Pledge-Manager)). What remains are creator actions rather than project attributes: rewards itemized, the plain Backer Survey not yet distributed, no other pledge manager service running alongside, and a campaign that ran on Kickstarter. **One-way door:** distributing the plain Backer Survey forecloses the Pledge Manager permanently — a creator may revert from Pledge Manager to survey-only before the survey launches, never the reverse. The confirming check is the creator nav after launch: eligible shows *Pledge manager*, ineligible shows *Backer survey* in the same slot.
...
So a slice of backers will hand over an address that is structurally unusable as a PRISM login. **Ask for the PRISM email explicitly as a survey question** rather than harvesting the account email, and expect a residue that the on-demand escape hatch must cover regardless of which mechanism is chosen. This is an argument for keeping *some* manual path open — not an argument for building it in Stripe. **Verified 2026-09-04:** it also constrains the *vehicle*. The survey has to be Kickstarter’s own — either the Pledge Manager’s or the plain Backer Survey, both in-platform — because the quote above rules out anything sent “via a third-party survey tool or pledge manager service”, which is exactly the cohort the question exists to serve. Both in-platform vehicles can carry the question, so the eligibility outcome above does not put this finding at risk; only a genuinely third-party tool does.
...
**Stamp Founders by email allowlist. Handle the tail with a manual stamp. Build nothing in Stripe.**

1. **Do not pass `discounts` or `subscription_data[trial_*]` in `stripe-checkout-edge.ts`.** Leave the Checkout call as it is. Founders should never reach Checkout at all; a user with an entitlement has no reason to start a subscription.
2. **Keep the Founder flag out of the `subscriptions` table's Stripe-shaped columns.** `subscriptions` is written exclusively by `stripe-webhook-edge.ts` from Stripe events. Writing a non-Stripe entitlement into a row whose `updated_at` is a Stripe event timestamp, and whose upsert logic is "update only if older," will be reverted by the next real webhook. This is #186's decision, but the constraint belongs on the record here: **the Founder flag needs its own storage.**
...
- ~~**Kickstarter Pledge Manager eligibility was not verified for PRISM’s project.** An eligibility article exists; its criteria were not read.~~ **Closed 2026-09-04** — the criteria were read and the gate no longer exists: the tool went to all creators on 2025-05-08 and all projects are eligible subject to creator actions only. See §2. Residual gap: the help-centre article was reachable only through the Zendesk article API and a search index, since `help.kickstarter.com/hc/` now 403s automated fetching too, and the creator-nav confirmation cannot be observed until the campaign launches.
